...
import {Coordinate, Entity, Tile} from "../board";
...
export type ActionStrategyCallable = (spell: Spell, by: Entity<Resources>, to: Tile[], args: ActionArgs) => ActionResume;
...
export const ACTION_STRATEGIES: Record<ActionStrategy, ActionStrategyCallable> = {
    [ActionStrategy.UPDATE_RESSOURCE]: (spell: Spell, by: Entity<Resources>, targets: Tile[], args: ActionArgs) => {
        ...
        for (let target of targets) {
            const VALUE = target.entity?.updateResource(
                args.type! as ResourceType,
                (value: number) => value + args.value!
            );
